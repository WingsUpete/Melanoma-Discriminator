...
#############################################################
# This file serves as the main for the whole module, refactored into
# the framework PyTorch Lightning: https://github.com/PyTorchLightning/pytorch-lightning
# More explanation article in Chinese: https://cloud.tencent.com/developer/article/1593703
...
class MelanomaDiscriminator(pl.LightningModule):

    def __init__(self, efnet_version=1, rs=Config.RESIZE_DEFAULT, dh=Config.DRAW_HAIR_DEFAULT, minibatch_size=Config.BATCH_SIZE_DEFAULT, \
                 num_workers=Config.WORKERS_DEFAULT, fd=None, optimizer=Config.OPTIMIZER_DEFAULT):
        """
        ...
        """
        super(MelanomaDiscriminator, self).__init__()
        
        if not os.path.isdir(Config.MODEL_DEFAULT):
            os.mkdir(Config.MODEL_DEFAULT)

        stdLog(sys.stdout, "Loading Melanoma Dataset...\n", DEBUG, fd)
        self.dataset = MelanomaDataSet(folder, train_transform=Config.get_train_transform(rs, bool(dh)), eval_transform=Config.get_eval_transform(rs))
        ...
        self.train_loss = 0
        self.train_total = len(self.dataset.trainset)
    ...
    def prepare_data(self):
        """
        Download and pre-process data. Nothing to do here
        """
        pass

    def train_dataloader(self):
        return DataLoader(self.dataset.trainset, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers)

    def val_dataloader(self):
        return DataLoader(self.dataset.validset, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)

    def test_dataloader(self):
        return DataLoader(self.dataset.testset, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)

    def configure_optimizers(self):
        if self.optimizer == 'ADAM':
            return torch.optim.Adam(net.parameters(), learning_rate, weight_decay=Config.WEIGHT_DECAY_DEFAULT)    # Adam + L2 Norm
        elif self.optimizer == 'RMSprop':
            return torch.optim.RMSprop(net.parameters(), learning_rate, weight_decay=Config.WEIGHT_DECAY_DEFAULT)
        else:
            return torch.optim.Adam(net.parameters(), learning_rate, weight_decay=Config.WEIGHT_DECAY_DEFAULT)    # Default: Adam + L2 Norm
    ...
    def training_step(self, batch, batch_idx):
        ...
        res = self.forward(samples)              # [[1], [2], [3]]
        loss = self.criterion(res, labels.type_as(res).reshape(-1, 1))  # BCEWithLogitsLoss does not support Long
        
        preds = torch.round(torch.sigmoid(res)) # set threshold to be 0.5 so that values below 0.5 will be considered 0
        self.train_correct += (preds == labels.type_as(res).reshape(-1, 1)).sum().item()
        self.train_loss += loss.item()

    def on_train_end(self, trainer, pl_module):
        train_acc = self.train_correct / self.train_total
        stdLog(sys.stdout, 'Training Round %d: acc = %.2f%%, loss = %.2f\n' % (self.current_epoch, train_acc * 100, ))
